...
# Write results
write.csv(as.data.frame(resOrdered), file = "DESeq2_results_ROOT_ALL.csv")
...
###Make the heatmap
library(pheatmap)
png("ROOT_ALL_HEATMAP_genes.png", height = 5, width = 7, units = 'in', res = 600)
pheatmap(vst_cor_root, annotation = select(samples, species))
dev.off()

###Make the PCA
library(ggplot2)
png("root_PCA_GENES.png", height = 4, width = 6, units = 'in', res = 600)
...
png("mean_variance_plot_ROOT.png", height = 4, width = 6, units = 'in', res = 600)
...
png("mean_variance_plot_ROOT_normalized.png", height = 4, width = 6, units = 'in', res = 600)
...
png("Dispersion_ROOT.png", height = 4, width = 6, units = 'in', res = 600)
...
png("MA_PLOT_0.05_ROOT.png", height = 4, width = 6, units = 'in', res = 600)
...
##Run heatmap
png("ROOT_heatmap_Significant_DE_Genes.png", height = 5, width = 5, units = 'in', res = 600)
...
# Basic volcano plot
png("ROOT_VOLCANO_Significant_DE_Genes.png", height = 5, width = 7, units = 'in', res = 600)
...
png("DE_LEAF_volcano_top15_labeled_geneNAMES.png", height = 5, width = 7,units = 'in', res = 600)
...
##Expression Plot
top_25<-data.frame(norm_leaf_counts) [1:25,] %>%
  rownames_to_column(var="trinity_gene")
top_25<-gather(top_25, key="samplename", value = "normalized_counts", 2:9)
top_25<-inner_join(top_25, rownames_to_column(samples, var="samplename"), by="samplename")

Species <- factor(top_25$condition, level = c('drought', 'water'))
Colors<-c("darkcyan", "cyan1")
